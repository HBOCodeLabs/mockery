...
func NewGenerator(iface *Interface, pkg string, inPackage bool, pkgPrefix bool) *Generator {
	var roots []string
...
func pkgNameCamel(pkg, name string) string {
	if pkg != "" {
		pkg = strings.ToUpper(pkg[0:1]) + pkg[1:len(pkg)]
	}

	if name != "" {
		name = strings.ToUpper(name[0:1]) + name[1:len(name)]
	}
...
func (g *Generator) mockName() string {
	...
	if g.pkgPrefix {
		return pkgNameCamel(g.iface.Pkg.Name(), g.iface.Name)
	}
	return g.iface.Name
}
...
func (g *Generator) Generate() error {
	...
	for i := 0; i < g.iface.Type.NumMethods(); i++ {
		...
		g.printf(
			"func (_m *%s) %s(%s) ", g.iface.MockName, fname,
